<script lang="ts">
	...
	function sendPing(): void {
		try {
			...
			addMessage(
				"received",
				`Error sending ping: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
		...
		try {
			if (ws) {
				...
				if (success) {
					...
					addMessage(
						"received",
						"Failed to request status - WebSocket not connected",
					);
				}
			} else {
				addMessage("received", "WebSocket service not available");
			...
			addMessage(
				"received",
				`Error requesting status: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
		...
		try {
			...
			if (ws && typeof ws.send === "function") {
				...
				if (success) {
					...
					addMessage(
						"received",
						"Failed to send custom message - WebSocket not connected",
					);
				}
			} else {
				addMessage("received", "WebSocket service not available");
			...
			addMessage(
				"received",
				`Error sending custom message: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	function reconnect(): void {
		try {
			if (ws) {
				...
				try {
					if (typeof ws.connect === "function") {
						...
						addMessage(
							"received",
							"WebSocket connect method not available",
						);
					}
				} catch (error) {
			...
			addMessage(
				"received",
				`Error initiating reconnect: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	onMount(() => {
		try {
			if (ws && typeof ws.on === "function") {
				wsOff = ws.on((evt: WebSocketEvent) => {
					try {
						...
						addMessage(
							"received",
							`Error handling WebSocket event: ${error instanceof Error ? error.message : "Unknown error"}`,
						);
					}
				});
			} else {
			...
			addMessage(
				"received",
				`Component initialization error: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	});
		...
		try {
			if (wsOff) {
				try {
					...
					console.warn(
						"Error cleaning up WebSocket listener:",
						error,
					);
				}
				wsOff = null;
	...
	<div class="card-body">
		...
		<div class="card bg-base-100">
			<div class="form-control p-2">
				...
				<div
					id="message-log"
					class="bg-base-100 p-2 rounded-lg h-64 overflow-y-auto font-mono text-xs"
				>
							...
							>
								...
								>
									<span
										...
										class:badge-primary={msg.type ===
											"sent"}
										class:badge-secondary={msg.type ===
											"received"}
									>
										{msg.type}
									</span>
								...
								<pre
									class="whitespace-pre-wrap break-all">{msg.content}</pre>
							</div>
						{/each}
					{/if}
				</div>
